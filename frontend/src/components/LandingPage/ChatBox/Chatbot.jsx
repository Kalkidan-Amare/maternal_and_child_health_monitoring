  ...
  return (
    <Drawer direction="left">
      <DrawerTrigger asChild>
        <Button
          className={cn("bg-emerald-600 rounded-full fixed bottom-4 left-4 ")}
        >
          <FaRobot />
        </Button>
      </DrawerTrigger>
      <DrawerContent
        className={cn(
          " w-full sm:w-10/12 md:w-1/2 lg:w-2/6 overflow-scroll p-2 h-full"
        )}
      >
        <DrawerHeader>
          <span>
        ...
        <div className="chat-container">
          <div className="w-full">
            {messages.map((message, index) => (
              <Message
                key={index}
                sender={message.role === "user" ? true : false}
        ...
        <DrawerFooter>
          <form className="flex" onSubmit={handleSubmit}>
            <Input
              required
              type="text"
              className={cn("w-10/12 mr-3 focus:border-0 focus:outline-primary ")}
